...
verifies that all three skills, the deployment-local OpenAPI document, authorization-server metadata,
and protected-resource metadata agree on the canonical installation URLs. It also verifies the
retirement notice at both old instruction paths.
